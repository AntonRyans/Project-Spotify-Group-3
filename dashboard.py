...
def advanced_dashboard(artists, tracks, albums, features):
    ...
    filtered_albums = albums[
        (albums["year"] >= year_range[0]) & (albums["year"] <= year_range[1])
    ]
    ...
    with c1:
        ...
        if not filtered_tracks_data.empty:
            exp_pop = (
                filtered_tracks_data.groupby("explicit")["track_popularity"]
                .mean()
                .reset_index()
            )
            colors = [
                "#1DB954" if x == "false" else "#191414" for x in exp_pop["explicit"]
            ]
            fig, ax = plt.subplots()
    ...
    with c2:
        ...
        if not filtered_tracks_data.empty:
            explicit_stats = (
                filtered_tracks_data.groupby("artist_0")["explicit"]
...
def main():
    ...
    page = st.sidebar.radio(
        "Select Dashboard Page:", ["Basic Dashboard", "Advanced Dashboard"]
    )
